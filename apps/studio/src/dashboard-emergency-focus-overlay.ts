...
export class FrakonDashboardEmergencyFocusOverlay extends LitElement {
  ...
  static styles = css`
    ...
    .icon { grid-row:1 / span 4; display:grid; place-items:center; width:24px; height:24px; border-radius:7px; background:rgb(0 0 0 / 20%); font-size:16px; font-weight:900; }
    .label strong { font-weight:900; letter-spacing:.03em; }
    .label .source { font-size:11px; opacity:.94; }
    .label .guidance { margin-top:2px; font:600 10px/1.35 Inter,system-ui,sans-serif; opacity:.9; }
    .label .technical { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; opacity:.62; font:500 9px/1.25 ui-monospace,SFMono-Regular,monospace; }
  ...
  render() {
    ...
    return html`<div class="veil"></div>${this.focusState.targets.map((target) => {
      ...
      return html`<div class=${`focus ${presentation.kind} ${active ? 'active' : 'muted'}`} style=${style}><div class="label"><span class="icon">${presentation.icon}</span><strong>${strings.critical.toUpperCase()} · ${presentation.title}</strong>${presentation.sourceLabel ? html`<span class="source">${presentation.sourceLabel}</span>` : nothing}<span class="guidance">${presentation.guidance}</span>${presentation.sourceEntityId ? html`<span class="technical">${presentation.sourceEntityId}</span>` : nothing}</div></div>`;
    })}`;
  }
}
