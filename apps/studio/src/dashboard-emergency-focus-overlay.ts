...
export class FrakonDashboardEmergencyFocusOverlay extends LitElement {
  ...
  static styles = css`
    ...
    .focus { position:absolute; box-sizing:border-box; border:3px solid rgb(255 78 91 / 96%); border-radius:14px; background:rgb(255 78 91 / 8%); box-shadow:0 0 0 7px rgb(255 78 91 / 16%),0 0 42px rgb(255 78 91 / 48%); animation:pulse 1.25s ease-in-out infinite alternate; transition:opacity 140ms ease,filter 140ms ease,box-shadow 140ms ease; }
    .label { position:absolute; left:8px; top:8px; display:grid; gap:2px; max-width:calc(100% - 16px); padding:6px 8px; border-radius:8px; color:white; background:rgb(151 19 31 / 94%); font:700 10px/1.25 Inter,system-ui,sans-serif; }
    ...
    .focus.active { z-index:2; box-shadow:0 0 0 8px rgb(255 78 91 / 22%),0 0 54px rgb(255 78 91 / 64%); filter:brightness(1.16); }
  ...
  render() {
    ...
    return html`<div class="veil"></div>${this.focusState.targets.map((target) => {
      ...
      return html`<div class=${`focus ${active ? 'active' : 'muted'}`} style=${style}><div class="label"><strong>CRITICAL · ${target.itemId}</strong>${target.reasons[0] ? html`<span>${target.reasons[0]}</span>` : nothing}${target.sourceEntityIds.length > 0 ? html`<small>${target.sourceEntityIds.join(', ')}</small>` : nothing}</div></div>`;
    })}`;
  }
}
