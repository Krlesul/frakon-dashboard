...
export class FrakonDashboardEmergencyFocusOverlay extends LitElement {
  ...
  static styles = css`
    ...
    .focus.smoke { --tone:255 92 92; }
    .focus.gas { --tone:255 170 72; }
    .focus.water { --tone:74 165 255; }
    .focus.safety { --tone:255 196 74; }
    .focus.alarm { --tone:255 62 120; }
    .focus.battery { --tone:255 145 64; }
    .focus.climate { --tone:193 105 255; }
    .focus.unavailable { --tone:166 176 190; }
    .label { position:absolute; left:8px; top:8px; display:grid; grid-template-columns:auto minmax(0,1fr); gap:3px 7px; align-items:center; max-width:calc(100% - 16px); padding:7px 9px; border-radius:8px; color:white; background:rgb(var(--tone) / 88%); font:700 10px/1.25 Inter,system-ui,sans-serif; }
    .icon { grid-row:1 / span 3; display:grid; place-items:center; width:24px; height:24px; border-radius:7px; background:rgb(0 0 0 / 20%); font-size:16px; font-weight:900; }
    .label strong { font-weight:900; letter-spacing:.03em; }
    .label .source { font-size:11px; opacity:.94; }
    .label .technical { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; opacity:.68; font:500 9px/1.25 ui-monospace,SFMono-Regular,monospace; }
    .focus.active { z-index:2; box-shadow:0 0 0 8px rgb(var(--tone) / 22%),0 0 54px rgb(var(--tone) / 64%); filter:brightness(1.16); }
  ...
  render() {
    ...
    return html`<div class="veil"></div>${this.focusState.targets.map((target) => {
      ...
      return html`<div class=${`focus ${presentation.kind} ${active ? 'active' : 'muted'}`} style=${style}><div class="label"><span class="icon">${presentation.icon}</span><strong>${strings.critical.toUpperCase()} · ${presentation.title}</strong>${presentation.sourceLabel ? html`<span class="source">${presentation.sourceLabel}</span>` : nothing}${presentation.sourceEntityId ? html`<span class="technical">${presentation.sourceEntityId}</span>` : nothing}</div></div>`;
    })}`;
  }
}
