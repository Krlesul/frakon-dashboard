...
export class FrakonDashboardConflictCanvasOverlay extends LitElement {
  ...
  static styles = css`
    ...
    .box {
      ...
      border-radius:10px;
      transition:left 160ms ease,top 160ms ease,width 160ms ease,height 160ms ease,opacity 120ms ease;
    }
    .box::after {
  ...
  private renderBox(item: FrakonGridItem, kind: 'local' | 'remote' | 'resolved', label: string, muted: boolean) {
    ...
    return html`<div class=${`box ${kind} ${muted ? 'muted' : ''}`} data-label=${label} style=${style}></div>`;
  }
}
