...
export class FrakonDashboardEmergencyAuditPanel extends LitElement {
  ...
  render(){
    const s=this.strings(); const filtered=this.filtered(); const stats=calculateDashboardEmergencyHistoryStats(filtered); const total=stats.totalEvents; const trend7=calculateDashboardEmergencyHistoryTrend(filtered,7); const trend30=calculateDashboardEmergencyHistoryTrend(filtered,30);
