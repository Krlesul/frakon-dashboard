...
export class FrakonDashboardIntelligenceLivePanel extends LitElement {
  ...
  static styles = css`
    ...
    .emergency-controls {
      ...
      margin:-2px 0 10px;
      padding:10px 11px;
      border:1px solid rgb(255 91 91 / 32%);
      border-radius:11px;
  ...
  private onContextChanged(event: CustomEvent<FrakonDashboardIntelligenceContextChangedDetail>): void {
    ...
    if (!focus.targets.some((target) => target.itemId === this.emergencyActiveItemId)) {
  ...
  private currentEmergencyItemId(focus: DashboardEmergencyFocusState): string {
    if (this.emergencyActiveItemId && focus.targets.some((target) => target.itemId === this.emergencyActiveItemId)) {
      return this.emergencyActiveItemId;
    }
    return focus.primary?.itemId ?? '';
  }

  private restoreEmergencyView(): void {
    this.emergencyRestoreToken += 1;
  }

  private refocusEmergencyView(): void {
    this.emergencyFocusToken += 1;
  }
  ...
  private renderEmergencyControls(focus: DashboardEmergencyFocusState) {
    ...
    return html`
      <div class="emergency-controls" role="group" aria-label="Emergency Focus navigation">
        <strong>Critical ${Math.max(0, index) + 1} of ${focus.targets.length} · ${currentId}</strong>
        <button ?disabled=${focus.targets.length <= 1} @click=${() => this.moveEmergencyFocus(-1)}>Previous</button>
        <button ?disabled=${focus.targets.length <= 1} @click=${() => this.moveEmergencyFocus(1)}>Next</button>
        <button @click=${this.restoreEmergencyView}>Return to previous view</button>
        <button class="primary" @click=${this.refocusEmergencyView}>Focus again</button>
      </div>
    `;
  }
  ...
  render() {
    ...
    return html`
      ...
      <frakon-dashboard-emergency-focus-bridge
        ...
        .activeItemId=${emergencyItemId}
        .focusToken=${this.emergencyFocusToken}
        .restoreToken=${this.emergencyRestoreToken}
      ></frakon-dashboard-emergency-focus-bridge>
      ${this.renderStatus()}
      ${this.renderEmergencyControls(emergencyFocus)}
