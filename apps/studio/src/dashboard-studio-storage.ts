...
export class FrakonDashboardStudioStorage extends LitElement {
  ...
  static styles = css`
    ...
    .dot.pending { background:#ffcf66; }
    .dot.saving { background:#69a7ff; box-shadow:0 0 0 4px rgb(105 167 255 / 12%); }
  ...
  private statusClass(): string {
    if (this.autosaveState.error) return 'error';
    ...
    return html`
      <section class="shell">
        <div class="storage-bar" role="status" aria-live="polite">
          <div class=${`status ${this.autosaveState.error ? 'error' : ''}`}>
            <span class=${`dot ${this.statusClass()}`}></span>
            <span class="label">${this.statusLabel()}</span>
          </div>
          <div class="actions">
            ${this.autosaveState.error ? html`<button @click=${this.retry}>Retry</button>` : nothing}
            <button
