...
import type { FrakonSwitchCardConfig } from './switch-card';

interface ConfigChangedEventDetail { config: FrakonSwitchCardConfig; }

@customElement('frakon-switch-card-editor')
export class FrakonSwitchCardEditor extends LitElement {
  ...
  setConfig(config: FrakonSwitchCardConfig): void { this.config = { show_state: true, ...config }; }

  private updateConfig<K extends keyof FrakonSwitchCardConfig>(key: K, value: FrakonSwitchCardConfig[K]): void {
  ...
  private entities(): string[] {
    return Object.keys(this.hass?.states ?? {}).filter((entityId) => entityId.startsWith('switch.')).sort();
  }
...
declare global { interface HTMLElementTagNameMap { 'frakon-switch-card-editor': FrakonSwitchCardEditor; } }
