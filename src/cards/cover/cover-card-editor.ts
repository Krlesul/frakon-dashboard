...
import type { FrakonCoverCardConfig } from './cover-card';

interface ConfigChangedEventDetail { config: FrakonCoverCardConfig; }

@customElement('frakon-cover-card-editor')
export class FrakonCoverCardEditor extends LitElement {
  ...
  setConfig(config: FrakonCoverCardConfig): void { this.config = { show_position: true, ...config }; }

  private updateConfig<K extends keyof FrakonCoverCardConfig>(key: K, value: FrakonCoverCardConfig[K]): void {
  ...
  private entities(): string[] {
    return Object.keys(this.hass?.states ?? {}).filter((entityId) => entityId.startsWith('cover.')).sort();
  }
  ...
  render() {
    ...
    return html`<div class="editor">
      ...
      <section class="section">
        ...
        <label class="toggle"><span>Show position</span><input type="checkbox" .checked=${this.config.show_position !== false} @change=${(event: Event) => this.updateConfig('show_position', (event.target as HTMLInputElement).checked)}></label>
      </section>
    </div>`;
  }
}

declare global { interface HTMLElementTagNameMap { 'frakon-cover-card-editor': FrakonCoverCardEditor; } }
