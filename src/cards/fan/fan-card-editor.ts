...
import type { FrakonFanCardConfig } from './fan-card';

interface ConfigChangedEventDetail { config: FrakonFanCardConfig; }

@customElement('frakon-fan-card-editor')
export class FrakonFanCardEditor extends LitElement {
  ...
  setConfig(config: FrakonFanCardConfig): void { this.config = { show_percentage: true, ...config }; }

  private updateConfig<K extends keyof FrakonFanCardConfig>(key: K, value: FrakonFanCardConfig[K]): void {
  ...
  private entities(): string[] {
    return Object.keys(this.hass?.states ?? {}).filter((entityId) => entityId.startsWith('fan.')).sort();
  }
  ...
  render() {
    ...
    return html`<div class="editor">
      ...
      <section class="section">
        ...
        <label class="toggle"><span>Percentage speed control</span><input type="checkbox" .checked=${this.config.show_percentage !== false} @change=${(event: Event) => this.updateConfig('show_percentage', (event.target as HTMLInputElement).checked)}></label>
      </section>
    </div>`;
  }
}

declare global { interface HTMLElementTagNameMap { 'frakon-fan-card-editor': FrakonFanCardEditor; } }
