...
import type { FrakonMediaPlayerCardConfig } from './media-player-card';

interface ConfigChangedEventDetail {
  config: FrakonMediaPlayerCardConfig;
}

@customElement('frakon-media-player-card-editor')
export class FrakonMediaPlayerCardEditor extends LitElement {
  ...
  private updateConfig<K extends keyof FrakonMediaPlayerCardConfig>(
    key: K,
    value: FrakonMediaPlayerCardConfig[K],
  ): void {
  ...
  render() {
    ...
    const entities = this.mediaEntities();
    return html`
      <div class="editor">
        ...
        <section class="section">
          ...
          <label class="toggle">
            ...
            <input
              type="checkbox"
              .checked=${this.config.show_volume !== false}
              @change=${(event: Event) => this.updateConfig('show_volume', (event.target as HTMLInputElement).checked)}
            >
          </label>
        </section>
      </div>
...
declare global {
  interface HTMLElementTagNameMap {
    'frakon-media-player-card-editor': FrakonMediaPlayerCardEditor;
  }
}
