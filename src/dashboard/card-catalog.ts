...
export const frakonCardCatalog: readonly FrakonCardTemplate[] = [
  ...
  { type:'custom:frakon-switch-card', name:'Switch', description:'Direct on/off control for Home Assistant switches.', category:'general', defaultWidth:3, defaultHeight:3, createConfig:(entity='switch.placeholder')=>({ type:'custom:frakon-switch-card', entity, show_state:true }) },
  { type:'custom:frakon-action-card', name:'Action', description:'Press a button, run a script or activate a scene.', category:'general', defaultWidth:3, defaultHeight:3, createConfig:(entity='button.placeholder')=>({ type:'custom:frakon-action-card', entity, show_state:false }) },
  ...
  { type:'custom:frakon-fan-card', name:'Fan', description:'Fan power and percentage speed control.', category:'climate', defaultWidth:3, defaultHeight:4, createConfig:(entity='fan.placeholder')=>({ type:'custom:frakon-fan-card', entity, show_percentage:true }) },
  { type:'custom:frakon-binary-sensor-card', name:'Binary sensor', description:'Door, window, motion, smoke, moisture and safety state card.', category:'security', defaultWidth:3, defaultHeight:3, createConfig:(entity='binary_sensor.placeholder')=>({ type:'custom:frakon-binary-sensor-card', entity, show_state:true }) },
  ...
  { type:'custom:frakon-lock-card', name:'Lock', description:'Lock and unlock control with state feedback.', category:'security', defaultWidth:3, defaultHeight:3, createConfig:(entity='lock.placeholder')=>({ type:'custom:frakon-lock-card', entity, show_state:true, confirm_unlock:true }) },
