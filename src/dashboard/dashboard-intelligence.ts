...
export function analyzeDashboardIntelligence(
  document: FrakonDashboardDocument,
  context: DashboardIntelligenceContext,
): DashboardIntelligenceAnalysis {
...
function scoreItem(
  ...
  if (usageBoost > 0) {
    score += usageBoost;
    reasons.push(`recent usage adds ${usageBoost} points`);
  }
  ...
  if (deviceBoost !== 0) {
    score += deviceBoost;
    reasons.push(`${context.device} context ${deviceBoost > 0 ? 'adds' : 'removes'} ${Math.abs(deviceBoost)} points`);
  }

  const daypartBoost = daypartAdjustment(type, context.daypart);
  if (daypartBoost !== 0) {
    score += daypartBoost;
    reasons.push(`${context.daypart} context adds ${daypartBoost} points`);
  }
  ...
  return {
    itemId: item.id,
    score,
    tier: score >= 100 ? 'critical' : score >= 70 ? 'primary' : score >= 40 ? 'secondary' : 'background',
    reasons,
    recommendedWidth: recommendedWidth(item, type, context.device, base.preferredWidth),
    recommendedHeight: recommendedHeight(item, type, context.device, base.preferredHeight),
  };
}
