...
export class FrakonResponsiveV2SavePanel extends LitElement {
  ...
  private requestSave(): void {
    if (!this.preview?.wouldWrite) return;
  ...
  render() {
    ...
    return html`<section class="panel">
      ...
      ${preview.readiness.blockers.length ? html`<div class="blockers">
        ${preview.readiness.blockers.map((blocker) => html`<div class="blocker">${this.t(blocker)}</div>`)}
      </div>` : nothing}
      <button ?disabled=${!preview.wouldWrite} @click=${this.requestSave}>${this.t('save')}</button>
    </section>`;
  }
}
