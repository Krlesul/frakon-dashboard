...
export class FrakonDashboardCard extends LitElement {
  ...
  @state() private pointerGuidelines: Guideline[] = [];
  ...
  private pointerMovingIds: string[] = [];
  private marqueeSession?: DashboardMarqueeSession;
  private marqueePointerId?: number;

  static styles = css`
    :host { display: block; }
    .shell { padding: 16px; border-radius: 24px; background: var(--card-background-color); outline: none; }
    ...
    .selection-badge { background: color-mix(in srgb, var(--primary-color) 24%, transparent); }
    ...
    .grid { display: grid; position: relative; align-items: stretch; min-height: 48px; }
    ...
    .marquee { position: absolute; z-index: 20; pointer-events: none; border: 1px solid var(--primary-color); background: color-mix(in srgb, var(--primary-color) 14%, transparent); box-shadow: 0 0 0 1px color-mix(in srgb, var(--primary-color) 15%, transparent) inset; }
  ...
  static getConfigElement(): HTMLElement {
    return document.createElement('frakon-dashboard-card-editor');
  }
  ...
  protected updated(changed: PropertyValues<this>): void {
    if (!changed.has('hass') || !this.config) return;
    const storageChanged = this.configureStorage(this.config.storage ?? 'local');
    if (storageChanged) void this.loadStoredDocument(this.config.dashboard_id ?? 'default');
  }
  ...
  private subscribeToStorage(): void {
    this.unsubscribeStorage?.();
    this.unsubscribeStorage = this.storageController.subscribe((storageState) => {
      this.storageState = storageState;
    });
  }

  private configureStorage(mode: DashboardStorageMode): boolean {
    const next = createHomeAssistantDashboardStorage(mode, this.hass);
    if (next.kind === this.storageController.adapterKind) return false;
  ...
  private selectItem(event: MouseEvent, item: FrakonGridItem): void {
    this.selection = selectDashboardCard(this.selection, item.id, {
      shiftKey: event.shiftKey,
      ctrlKey: event.ctrlKey,
      metaKey: event.metaKey,
    });
    this.selectedId = this.selection.anchorId;
  }
  ...
  private resize(item: FrakonGridItem, dw: number, dh: number): void {
    if (!this.document || this.pointerSession || this.marqueeSession) return;
    this.clearAutoLayoutPreview();
    this.persist(updateGridItemCollisionSafe(this.document, item.id, { w: item.w + dw, h: item.h + dh }));
  }

  private pointerPoint(event: PointerEvent): DashboardPointerPoint {
    return { x: event.clientX, y: event.clientY };
  }

  private pointerContainerWidth(): number {
    return this.renderRoot.querySelector<HTMLElement>('.grid')?.getBoundingClientRect().width ?? this.containerWidth;
  }

  private updatePointerPreview(event: PointerEvent): void {
    if (!this.pointerSession || !this.document) return;
    const preview = this.pointerSession.preview(this.pointerPoint(event));
    this.pointerPreview = preview;
    this.pointerGuidelines = this.pointerMovingIds.length
      ? dashboardPointerGuidelines(this.document, preview.items, this.pointerMovingIds, this.pointerContainerWidth())
      : [];
  }

  private beginPointerMove(event: PointerEvent, item: FrakonGridItem): void {
    if (!this.document || item.locked || this.autoLayoutPreview || this.pointerSession || this.marqueeSession || event.button !== 0) return;
    event.preventDefault();
    event.stopPropagation();
    const selectedIds = dashboardPointerMoveSelection(this.selection, item, this.document.items);
    ...
    this.clearAutoLayoutPreview();
    this.pointerMovingIds = [...selectedIds];
    this.pointerSession = new DashboardPointerSession(
      this.document,
      { kind: 'move', selectedIds },
      this.pointerPoint(event),
      this.pointerContainerWidth(),
    );
  ...
  private beginPointerResize(event: PointerEvent, item: FrakonGridItem): void {
    ...
    this.selectedId = item.id;
    this.clearAutoLayoutPreview();
    this.pointerMovingIds = [];
    this.pointerGuidelines = [];
    this.pointerSession = new DashboardPointerSession(
      this.document,
    ...
    this.pointerId = event.pointerId;
    this.pointerPreview = this.pointerSession.preview(this.pointerPoint(event));
    (event.currentTarget as HTMLElement).setPointerCapture?.(event.pointerId);
  }

  private onPointerMove(event: PointerEvent): void {
    if (this.pointerSession && this.pointerId === event.pointerId) {
      event.preventDefault();
      this.updatePointerPreview(event);
      return;
    }
    if (this.marqueeSession && this.marqueePointerId === event.pointerId) {
      event.preventDefault();
      const preview = this.marqueeSession.preview(this.marqueePoint(event));
      this.marqueeRect = preview.rect;
  ...
  private endPointerInteraction(event: PointerEvent): void {
    ...
    if (this.marqueeSession && this.marqueePointerId === event.pointerId) {
      event.preventDefault();
      this.selection = this.marqueeSession.commit(this.marqueePoint(event));
      this.selectedId = this.selection.anchorId;
      this.marqueeSession = undefined;
      this.marqueePointerId = undefined;
      this.marqueeRect = undefined;
    }
  }

  private marqueePoint(event: PointerEvent): DashboardMarqueePoint {
  ...
  private beginMarquee(event: PointerEvent): void {
    if (!this.document || this.config?.edit_mode !== true || this.autoLayoutPreview || this.pointerSession || this.marqueeSession || event.button !== 0 || event.target !== event.currentTarget) return;
    event.preventDefault();
    const additive = event.shiftKey || event.ctrlKey || event.metaKey;
    this.marqueeSession = new DashboardMarqueeSession(
      this.document,
      this.marqueePoint(event),
      this.pointerContainerWidth(),
      this.selection,
      additive,
    );
  ...
  private cancelPointerInteraction(): void {
    ...
    this.pointerPreview = undefined;
    this.pointerMovingIds = [];
    this.pointerGuidelines = [];
  }

  private cancelMarquee(): void {
    if (this.marqueeSession) {
      this.selection = this.marqueeSession.cancel();
      this.selectedId = this.selection.anchorId;
    }
    this.marqueeSession = undefined;
    this.marqueePointerId = undefined;
  ...
  private addTemplate(event: CustomEvent<FrakonCardTemplateSelectedDetail>): void {
    ...
    const y = this.document.items.reduce((maximum, candidate) => Math.max(maximum, candidate.y + candidate.h), 0);
    ...
    this.selection = { ids: [id], anchorId: id };
    this.selectedId = id;
  ...
  private removeItem(item: FrakonGridItem): void {
    ...
    const ids = this.selection.ids.filter((id) => id !== item.id);
    this.selection = {
      ids,
      anchorId: this.selection.anchorId === item.id ? ids.at(-1) : this.selection.anchorId,
    };
    this.selectedId = this.selection.anchorId;
  }
  ...
  private beginAutoLayout(): void {
    if (!this.document || !this.document.items.length || this.pointerSession || this.marqueeSession) return;
    this.cancelInteractions();
  ...
  private async uploadImport(event: Event): Promise<void> {
    ...
    if (!file || this.pointerSession || this.marqueeSession) return;
    try {
  ...
  private marqueeStyle(): string | undefined {
    ...
    return `left:${left}px;top:${top}px;width:${Math.abs(width)}px;height:${Math.abs(height)}px`;
  }

  private renderedGuidelines(): Guideline[] {
    const seen = new Set<string>();
    return this.pointerGuidelines.filter((guideline) => {
  ...
  render() {
    const pointerDocument = this.pointerPreview && this.document
      ? { ...this.document, items: this.pointerPreview.items }
      : undefined;
    ...
    const collisions = new Set(this.pointerPreview?.collisionIds ?? []);
    const selectedIds = new Set(this.selection.ids);
    const interactionActive = Boolean(this.pointerSession || this.marqueeSession);
    ...
    return html`
      <section
        ...
        @pointerup=${this.endPointerInteraction}
        @pointercancel=${this.cancelInteractions}
      >
        <header>
          <h2>${doc.title}</h2>
          <div class="actions">
            ...
            ${editMode && this.selection.ids.length > 1
              ? html`<span class="badge selection-badge">${this.selection.ids.length} selected</span>`
              : nothing}
            ${editMode ? html`
              <button ?disabled=${!this.history?.canUndo || Boolean(preview) || interactionActive} @click=${this.undo}>${editorTranslate(lang, 'undo')}</button>
              <button ?disabled=${!this.history?.canRedo || Boolean(preview) || interactionActive} @click=${this.redo}>${editorTranslate(lang, 'redo')}</button>
              <button class="primary" ?disabled=${interactionActive} @click=${() => { this.paletteOpen = !this.paletteOpen; }}>${editorTranslate(lang, this.paletteOpen ? 'closePalette' : 'addCard')}</button>
              <button ?disabled=${!doc.items.length || Boolean(preview) || interactionActive} @click=${this.beginAutoLayout}>${autoLayoutTranslate(lang, 'autoLayout')}</button>
              <button ?disabled=${interactionActive} @click=${this.downloadExport}>${editorTranslate(lang, 'export')}</button>
              <label class="file-label">${editorTranslate(lang, 'import')}<input type="file" accept="application/json,.json" ?disabled=${interactionActive} @change=${this.uploadImport}></label>
            ` : nothing}
          </div>
        </header>
        ...
        ${editMode && this.paletteOpen
          ? html`<frakon-card-palette .language=${lang} @frakon-card-template-selected=${this.addTemplate}></frakon-card-palette>`
          : nothing}
        ${selected && !preview && !interactionActive
          ? html`<frakon-item-inspector .item=${selected} .hass=${this.hass} .language=${lang} @frakon-item-config-changed=${this.updateItemCard} @frakon-item-inspector-close=${this.clearSelection}></frakon-item-inspector>`
          : nothing}

        <div class="grid" style=${style} @pointerdown=${this.beginMarquee}>
          ${!doc.items.length
            ...
            : doc.items.map((item) => html`
              ...
              >
                ${editMode && !preview ? html`
                  <div class="item-head">
                    <button class="item-id" @click=${(event: MouseEvent) => this.selectItem(event, item)}>${item.id}${item.locked ? ` · ${editorTranslate(lang, 'locked')}` : ''}</button>
                    <div class="controls">
                      ...
                      <button ?disabled=${interactionActive} @click=${() => this.toggleLock(item)}>${editorTranslate(lang, item.locked ? 'unlock' : 'lock')}</button>
                      <button ?disabled=${item.locked || interactionActive} @click=${() => this.resize(item, -1, 0)}>−W</button>
                      <button ?disabled=${item.locked || interactionActive} @click=${() => this.resize(item, 1, 0)}>+W</button>
                      <button ?disabled=${item.locked || interactionActive} @click=${() => this.resize(item, 0, -1)}>−H</button>
                      <button ?disabled=${item.locked || interactionActive} @click=${() => this.resize(item, 0, 1)}>+H</button>
                      <button class="danger" ?disabled=${item.locked || interactionActive} @click=${() => this.removeItem(item)}>${editorTranslate(lang, 'remove')}</button>
                    </div>
                  </div>
                  ${!item.locked ? html`<div class="resize-handle" title="Resize" @pointerdown=${(event: PointerEvent) => this.beginPointerResize(event, item)}></div>` : nothing}
                ` : nothing}
...
declare global {
  interface HTMLElementTagNameMap {
    'frakon-dashboard-card': FrakonDashboardCard;
  }
}
