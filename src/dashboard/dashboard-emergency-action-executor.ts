...
import{beginDashboardEmergencyIdempotentExecution,dashboardEmergencyIdempotencyKey,finishDashboardEmergencyIdempotentExecution,type DashboardEmergencyIdempotencyStore}from'./dashboard-emergency-idempotency';
...
export async function executeDashboardEmergencyPlan(plan:DashboardEmergencyExecutionPlan,adapter:DashboardEmergencyExecutionAdapter,snapshot:DashboardEmergencyExecutionSnapshot,current:DashboardEmergencyExecutionSnapshot,confirmed=false,idempotencyStore?:DashboardEmergencyIdempotencyStore):Promise<DashboardEmergencyExecutionResult>{
 ...
 const key=dashboardEmergencyIdempotencyKey(plan,snapshot);if(inFlight.has(plan.id))return{status:'duplicate',reason:'The same emergency action is already being executed.'};
 if(idempotencyStore&&!beginDashboardEmergencyIdempotentExecution(idempotencyStore,key))return{status:'duplicate',reason:'The same emergency action was already started or completed for this incident.'};
 ...
 inFlight.add(plan.id);try{await adapter.callService(call.domain,call.service,call.serviceData);if(idempotencyStore)finishDashboardEmergencyIdempotentExecution(idempotencyStore,key,'completed');return{status:'executed'};}catch(error){if(idempotencyStore)finishDashboardEmergencyIdempotentExecution(idempotencyStore,key,'failed');return{status:'failed',reason:error instanceof Error?error.message:String(error)};}finally{inFlight.delete(plan.id);}
}
