...
const KINDS: ConstraintKind[] = [
  'below', 'right-of', 'align-left', 'align-center-x', 'align-right', 'align-top', 'align-center-y', 'align-bottom', 'match-width', 'match-height',
];
...
export class FrakonCanvasV2ConstraintEditor extends LitElement {
  ...
  static styles = css`
    ...
    .row { display: grid; grid-template-columns: minmax(90px, 1.1fr) minmax(90px, 1.1fr) 76px 72px auto auto; gap: 6px; align-items: center; padding: 5px; border-radius: 10px; }
    .row.selected { background: color-mix(in srgb, var(--primary-color) 13%, transparent); outline: 1px solid color-mix(in srgb, var(--primary-color) 45%, transparent); }
  ...
  private applyResult(result: DashboardCanvasV2ConstraintEditResult): void {
    if (result.status === 'invalid') { this.localError = result.reason ?? 'Invalid constraint.'; return; }
    this.localError = result.status === 'collision' ? `Constraint edit blocked by collision: ${result.collisionIds.join(', ')}.` : undefined;
  ...
  render() {
    ...
    return html`<section class="section">
      ...
      ${constraints.map((constraint) => html`<div class="row ${constraint.id === this.selectedConstraintId ? 'selected' : ''}">
        <select title=${this.t('constraintKind')} .value=${constraint.kind} @change=${(event: Event) => this.patch(constraint.id, { kind: (event.currentTarget as HTMLSelectElement).value as ConstraintKind })}>${KINDS.map((kind) => html`<option value=${kind}>${kind}</option>`)}</select>
        <select title=${this.t('constraintTarget')} .value=${constraint.targetId} @change=${(event: Event) => this.patch(constraint.id, { targetId: (event.currentTarget as HTMLSelectElement).value })}>${targets.map((target) => html`<option value=${target.id}>${target.id}</option>`)}</select>
...
declare global { interface HTMLElementTagNameMap { 'frakon-canvas-v2-constraint-editor': FrakonCanvasV2ConstraintEditor; } }
