...
export function decideDashboardEmergencyRestartAction(audit:DashboardEmergencyActionAuditEntry|undefined,idempotency:DashboardEmergencyIdempotencyRecord|undefined):DashboardEmergencyRestartDecision{
 ...
 if(audit.verification?.status==='verified')return{kind:'completed',reason:'The physical state was already verified.'};
 ...
 if(audit.execution?.status==='failed')return{kind:'retry',reason:'The audited execution failed before completion.'};
 if(audit.execution?.status==='executed')return{kind:'verify-first',reason:'The audit records execution; verify physical state before retry.'};
