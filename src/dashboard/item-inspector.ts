...
export class FrakonItemInspector extends LitElement {
  ...
  render() {
    ...
    try { card = this.parsedDraft(); } catch { /* preserve visible form with last valid card */ }
    ...
    return html`
      <section class="panel">
        ...
        <div class="form">
          ...
          <label>Name<input .value=${String(card.name ?? '')} @input=${(event:Event) => this.updateField('name',(event.target as HTMLInputElement).value)}></label>
          <label>Icon<input placeholder="mdi:home" .value=${String(card.icon ?? '')} @input=${(event:Event) => this.updateField('icon',(event.target as HTMLInputElement).value)}></label>
        </div>
        <div class="advanced">
          <button @click=${() => { this.advancedOpen = !this.advancedOpen; }}>${this.advancedOpen ? 'Hide advanced JSON' : 'Show advanced JSON'}</button>
